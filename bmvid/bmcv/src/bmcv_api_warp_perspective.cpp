    ...
    bm_image*                      output)
{
    ...
    for (int i = 0; i < image_num; i++) {
        for (int j = 0; j < matrix[i].matrix_num; j++) {
            ...
            int left_top_x = (int)((m0 * 0 + m1 * 0 + m2) / (m6 * 0 + m7 * 0 + m8));
            int left_top_y = (int)((m3 * 0 + m4 * 0 + m5) / (m6 * 0 + m7 * 0 + m8));
            int left_btm_x = (int)((m0 * 0 + m1 * (image_dh - 1) + m2) / (m6 * 0 + m7 * (image_dh - 1) + m8));
            int left_btm_y = (int)((m3 * 0 + m4 * (image_dh - 1) + m5) / (m6 * 0 + m7 * (image_dh - 1) + m8));
            int right_top_x = (int)((m0 * (image_dw - 1) + m1 * 0 + m2) / (m6 * (image_dw - 1) + m7 * 0 + m8));
            int right_top_y = (int)((m3 * (image_dw - 1) + m4 * 0 + m5) / (m6 * (image_dw - 1) + m7 * 0 + m8));
            int right_btm_x = (int)((m0 * (image_dw - 1) + m1 * (image_dh - 1) + m2) / (m6 * (image_dw - 1) + m7 * (image_dh - 1) + m8));
            int right_btm_y = (int)((m3 * (image_dw - 1) + m4 * (image_dh - 1) + m5) / (m6 * (image_dw - 1) + m7 * (image_dh - 1) + m8));
