...
#define MIN_NUM_FREE_FB_REQUIRED 0 // TODO
...
BmJpuDecReturnCodes bm_jpu_calc_framebuffer_sizes(BmJpuColorFormat color_format,
                                    ...
                                    void *callback_user_data)
{
    ...
    *decoder = BM_JPU_ALLOC(sizeof(BmJpuDecoder));
    ...
    BM_JPU_DEBUG("opening decoder, frame size: %u x %u pixel",
                 open_params->frame_width, open_params->frame_height);
    BM_JPU_DEBUG("open_params->scale_ratio=%d\n", open_params->scale_ratio);
...
BmJpuDecReturnCodes bm_jpu_dec_close(BmJpuDecoder *decoder)
{
    ...
    if ((decoder->bs_dma_buffer != NULL) && (decoder->bs_virt_addr != NULL)) {
                                      ...
                                      unsigned int *output_code)
{
    ...
    bm_handle_t handle = bm_jpu_get_handle(decoder->device_index);
#ifdef BM_PCIE_MODE
    bm_memcpy_s2d_partial(handle, *(decoder->bs_dma_buffer), encoded_frame->data, encoded_frame->data_size);
#else
    ...
    memcpy(p_virt_addr, encoded_frame->data, encoded_frame->data_size);
    ...
    *output_code = BM_JPU_DEC_OUTPUT_CODE_INPUT_USED;
    ...
    BM_JPU_LOG("scale_width=%d, scale_height=%d\n", scale_width, scale_height);
         ...
         (decoder->old_jpeg_color_format != jpeg_color_format)))
    {
        ...
        initial_info.chroma_interleave = decoder->chroma_interleave;
        ...
        if (initial_info.roiFrameWidth > scale_width || initial_info.roiFrameHeight > scale_height){
    ...
    if (!(decoder->initial_info_available))
    {
        ...
        {
        case FORMAT_420:
            initial_info.color_format = BM_JPU_COLOR_FORMAT_YUV420;
            break;
        case FORMAT_422:
            initial_info.color_format = BM_JPU_COLOR_FORMAT_YUV422_HORIZONTAL;
            break;
        case FORMAT_224:
            initial_info.color_format = BM_JPU_COLOR_FORMAT_YUV422_VERTICAL;
            break;
        case FORMAT_444:
            initial_info.color_format = BM_JPU_COLOR_FORMAT_YUV444;
            break;
        case FORMAT_400:
            initial_info.color_format = BM_JPU_COLOR_FORMAT_YUV400;
            break;
        default:
...

    {
        ...
        timeout = jpu_DecWaitForInt(decoder->handle, JPU_WAIT_TIMEOUT, JPU_MAX_TIMEOUT_COUNTS);
                                      ...
                                      unsigned int *output_code)
{
    ...
    *output_code |= BM_JPU_ENC_OUTPUT_CODE_CONTAINS_HEADER;
    ...
    timeout = jpu_EncWaitForInt(encoder->handle, JPU_WAIT_TIMEOUT, JPU_MAX_TIMEOUT_COUNTS);
    ...
    BM_JPU_LOG("output info: bitstreamBuffer %" BM_JPU_PHYS_ADDR_FORMAT " bitstreamSize %u",
